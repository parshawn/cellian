...
const DrugTablet = ({ isInjecting, injectionProgress, keepVisible = false }: InjectionProps) => {
  ...
  // Make injection more visible with glow effect
  const injectionGlow = isInjecting ? 1.5 : 0.3;
  const injectionScale = isInjecting ? 1.3 : 1.0;

  return (
    <Float speed={1.5} rotationIntensity={0.3} floatIntensity={0.4}>
      <group ref={tabletRef} position={position} scale={injectionScale}>
        {/* Glow effect when injecting */}
        {isInjecting && (
          <mesh>
            ...
            <meshStandardMaterial
              color="#3b82f6"
              emissive="#3b82f6"
              emissiveIntensity={injectionGlow}
              transparent
              opacity={0.4}
            />
          </mesh>
...
const CRISPRComplex = ({ isInjecting, injectionProgress, keepVisible = false }: InjectionProps) => {
  ...
  return (
    <Float speed={1.5} rotationIntensity={0.3} floatIntensity={0.4}>
      <group ref={crisprRef} position={position} scale={injectionScale}>
        {/* Glow effect when injecting */}
        {isInjecting && (
          <mesh>
            <sphereGeometry args={[0.5, 16, 16]} />
