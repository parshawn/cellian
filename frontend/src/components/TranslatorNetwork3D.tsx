...
interface TranslatorNetwork3DProps {
  ...
  pipelineStage?: string; // Current pipeline stage: "perturbation", "rna", "protein", "analysis"
...
const NetworkScene = ({
  ...
  const isConnectionActive = (connectionId: string) => {
    ...
    // Fallback: Check if both start and end nodes are in active path
  ...
  return (
    <>
      ...
      <Node3D
        ...
        label={pipelineStage === "perturbation" && activePath.includes("drug") 
      ...
      <Node3D
        ...
        label={pipelineStage === "perturbation" && activePath.includes("perturbation")
          ? `Gene\n${currentStep || "Processing..."}`
          : "Gene Perturbation"}
        nodeId="perturbation"
        hoveredElement={hoveredElement}
      ...
      <Node3D
        ...
        label={pipelineStage === "rna" && activePath.includes("rna")
      ...
      <Node3D
        ...
        label={pipelineStage === "protein" && activePath.includes("protein")
          ? `Protein\n${currentStep || "Translating..."}`
          : pipelineStage === "analysis" && activePath.includes("protein")
          ? `Protein\nAnalyzing...`
          : "Protein"}
        nodeId="protein"
        hoveredElement={hoveredElement}
