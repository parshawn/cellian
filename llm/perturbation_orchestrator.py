        ...
        try:
            ...
            payload = {
                "context": {
                    ...
                    "user_question": query  # Pass the original user query to guide hypothesis generation
                },
...
def _generate_outputs(pert_results: Dict[str, Any], pert_type: str, pert_name: str, query_intent: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    # Separate RNA and protein pathways to avoid mixing them in plots
    pathways = []  # All pathways (for general use)
    rna_pathways = []  # RNA-specific pathways (for RNA GSEA plot)
    protein_pathways = []  # Protein-specific pathways (for protein PSEA plot)
    
    # RNA pathway sources
    rna_sources = ["gsea_rna", "kegg_enrichment_rna", "reactome_enrichment_rna", "go_enrichment_rna"]
    # Protein pathway sources
    protein_sources = ["kegg_enrichment", "reactome_enrichment", "go_enrichment"]
    # All sources
    all_sources = rna_sources + protein_sources
    
    for db in all_sources:
            ...
            try:
                ...
                if isinstance(enrichment, pd.DataFrame):
                    for _, row in enrichment.iterrows():
                        ...
                        # Add to RNA or protein specific lists
                        if db in rna_sources:
                            rna_pathways.append(pathway_dict)
                        elif db in protein_sources:
                            protein_pathways.append(pathway_dict)
                        
                count = len([p for p in pathways if p['source'] == db])
                print(f"  ✓ Loaded {count} pathways from {db}")
            except Exception as e:
    ...
    print(f"  Total pathways loaded: {len(pathways)} (RNA: {len(rna_pathways)}, Protein: {len(protein_pathways)})\n")
    ...
    # Filter RNA and protein pathways separately (for separate plots)
    filtered_rna_pathways = rna_pathways.copy()
    filtered_protein_pathways = protein_pathways.copy()
    if top_n_pathways:
        # Sort RNA pathways by absolute NES and take top N
        filtered_rna_pathways = sorted(rna_pathways, key=lambda x: abs(x.get("NES", 0.0)), reverse=True)[:top_n_pathways]
        # Sort protein pathways by absolute NES and take top N
        filtered_protein_pathways = sorted(protein_pathways, key=lambda x: abs(x.get("NES", 0.0)), reverse=True)[:top_n_pathways]
    ...
    # Separate pathways for different plots to avoid identical plots
    # For pathway_enrichment plot, use all pathways (or filtered if specified)
    plot_pathways = filtered_pathways if generate_pathways_plot else pathways
    # For RNA GSEA plot, use only RNA pathways (filtered if top_n_pathways specified)
    plot_rna_pathways = filtered_rna_pathways
    # For protein PSEA plot, use only protein pathways (filtered if top_n_pathways specified)
    plot_protein_pathways = filtered_protein_pathways
    ...
    if generate_pathways_plot and len(plot_pathways) > 0:
        ...
        # Generate RNA GSEA plot if we have both DEGs and RNA pathways (after pathway enrichment plot)
        if generate_genes_plot and len(plot_deg_list) > 0 and len(plot_rna_pathways) > 0:
            plot_num += 1
            try:
                print(f"  [{plot_num}/{total_plots}] Generating RNA GSEA plot (horizontal bar chart, RNA pathways only)...")
                plot_path = output_dir / "rna_gsea.png"
                outputs["rna_gsea"] = str(plot_rna_gsea(plot_deg_list, plot_rna_pathways, str(plot_path), top_pathways=plot_top_n_pathways, title_suffix=pert_label))
                print(f"    ✓ Saved: {outputs['rna_gsea']} (using {len(plot_rna_pathways)} RNA pathways)")
    ...
    if generate_proteins_plot and len(protein_deg_list) > 0 and len(plot_protein_pathways) > 0:
        plot_num += 1
        try:
            print(f"  [{plot_num}/{total_plots}] Generating protein PSEA plot (horizontal bar chart, protein pathways only)...")
            plot_path = output_dir / "protein_psea.png"
            # Use protein_deg_list and protein pathways only for protein PSEA
            outputs["protein_psea"] = str(plot_protein_psea(filtered_protein_list, validated_edges, plot_protein_pathways, str(plot_path), top_pathways=plot_top_n_pathways, title_suffix=pert_label))
            print(f"    ✓ Saved: {outputs['protein_psea']} (using {len(plot_protein_pathways)} protein pathways)")
        ...
        elif len(plot_protein_pathways) == 0:
            print(f"  ⚠️  Skipping protein PSEA plot: No protein pathway data available")
        outputs["protein_psea"] = None
